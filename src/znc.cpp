...
CString CZNC::MakeConfigHeader() {
	return
		"// WARNING\n"
		"//\n"
		"// Do NOT STOP ZNC While SQL is being populated!\n";
}
/* Making the new Config AKA Populating ZNC */
bool CZNC::WriteNewConfig(const CString& sConfigFile) {
	...
	sql = CUtils::GetBoolInput("Would you like to use MySQL instead of flatfile config?", sql);
	
	if (sql) {
	
		CUtils::PrintMessage("MySQL Configuration");
		CUtils::GetInput("MySQL Host", sListenHost, sListenHost, "Blank for all ips");
		CUtils::GetInput("MySQL Port", sListenHost, sListenHost, "3306 by Default");
		CUtils::GetInput("MySQL User", sListenHost, sListenHost, "Blank for all ips");
		CUtils::GetInput("MySQL Password", sListenHost, sListenHost, "Blank for all ips");
		CUtils::GetInput("MySQL Database", sListenHost, sListenHost, "Blank for all ips");
		
		CUtils::PrintMessage(" - Writing Config -");
		
		vsLines.push_back("<mysql>");
		vsLines.push_back("\tHost = " + CString(uListenPort));
...
bool CZNC::DoRehash(CString& sError)
{
	...
	if (!CFile::Exists(m_sConfigFile)) {
		...
		CUtils::PrintMessage("Restart ZNC with the --populate option to populate the MySQL Database");
		return false;
	}
